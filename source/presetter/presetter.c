...
void presetter_mousedown(t_presetter *p, t_object *patcherview, t_pt pt,
                         long modifiers);
void presetter_mouseup(t_presetter *p, t_object *patcherview, t_pt pt,
                       long modifiers);
void presetter_mousemove(t_presetter *p, t_object *patcherview, t_pt pt,
                         long modifiers);
void presetter_mouseleave(t_presetter *p, t_object *patcherview, t_pt pt,
                          long modifiers);

long presetter_key(t_presetter *p, t_object *patcherview, long keycode,
                   long modifiers, long textcharacter);
...
void ext_main(void *r) {
  ...
  c = class_new("presetter", (method)presetter_new, (method)presetter_free,
                sizeof(t_presetter), (method)NULL, A_GIMME, 0);
...
void presetter_get_init_button_dim(t_presetter *p, char *text, double *outwidth,
                                   double *outheight) {
  t_jsurface *surface =
      jgraphics_image_surface_create(JGRAPHICS_FORMAT_A8, 1, 1);
  t_jgraphics *g = jgraphics_create(surface);

  jgraphics_select_font_face(g, "Arial", JGRAPHICS_FONT_SLANT_NORMAL,
...
void presetter_init_preset_name_dim(t_presetter *p) {
  ...
  jgraphics_select_font_face(g, "Menlo", JGRAPHICS_FONT_SLANT_NORMAL,
                             JGRAPHICS_FONT_WEIGHT_BOLD);
...
void presetter_init_write_button_dim(t_presetter *p) {
  presetter_get_init_button_dim(p, WRITE_BUTTON_TEXT, &p->j_write_button_width,
...
t_presetter *presetter_new(t_symbol *s, short argc, t_atom *argv) {
  ...
  if (p) {
    long flags =
        0 | JBOX_DRAWFIRSTIN | JBOX_GROWBOTH | JBOX_NODRAWBOX | JBOX_HILITE;
...
void presetter_measure_text(t_jgraphics *g, char *text, double *outwidth,
                            double *outheight) {
  jgraphics_select_font_face(g, "Arial", JGRAPHICS_FONT_SLANT_NORMAL,
                             JGRAPHICS_FONT_WEIGHT_BOLD);
...
void presetter_measure_surface_text(t_presetter *p, char *text,
...
void presetter_handle_recall(t_presetter *p, long cell_idx,
  ...
  if (slot) {
    ...
    snprintf_zero(p->j_preset_name, sizeof(p->j_preset_name), "%s",
                  slot->s_name);

    if (update_status) {
      snprintf_zero(p->j_idle_status_text, sizeof(p->j_idle_status_text),
                    "Selected Preset %d", cell_idx);
    }
...
void presetter_handle_rename(t_presetter *p) {
  snprintf_zero(p->j_idle_status_text, sizeof(p->j_idle_status_text),
                "Renamed Preset %d", p->j_selected_cell);
  p->j_editing_name = false;
...
t_bounds presetter_get_preset_name_bounds(t_presetter *p, t_rect *rect) {
  ...
  bounds.width = rect->width - wbbounds.width - PRESET_NAME_OFFSET_X -
                 WRITE_BUTTON_OFFSET_X - PRESET_NAME_MARGIN_RIGHT;
...
t_grid_dim presetter_get_grid_dim(t_presetter *p, t_rect *rect) {
  t_grid_dim dim;
  dim.columns = (int)floor((rect->width - CELL_PADDING - GRID_OFFSET_X) /
                           CELL_TOTAL_SIZE);

  double grid_y_pos =
      PRESET_NAME_OFFSET_Y + p->j_preset_name_height + GRID_OFFSET_Y;
  double grid_space = rect->height - grid_y_pos - p->j_status_height -
                      STATUS_OFFSET_Y - CELL_PADDING;
...
bool presetter_in_grid_bounds(t_presetter *p, t_rect *rect, t_pt *pt) {
  ...
  bool in_x =
      pt->x >= bounds.x + CELL_PADDING && pt->x <= bounds.width + bounds.x;
  bool in_y =
      pt->y >= bounds.y + CELL_PADDING && pt->y <= bounds.height + bounds.y;
...
t_bounds presetter_get_confirm_ok_button_bounds(t_presetter *p, t_rect *rect,
  ...
  if (g) {
    presetter_measure_text(g, p->j_confirm_status_text, &text_width,
                           &text_height);
  } else {
    presetter_measure_surface_text(p, p->j_confirm_status_text, &text_width,
                                   &text_height);
  }
...
bool presetter_in_confirm_ok_button_bounds(t_presetter *p, t_rect *rect,
                                           t_pt *pt, t_jgraphics *g) {
...
t_bounds presetter_get_confirm_cancel_button_bounds(t_presetter *p,
                                                    t_rect *rect,
                                                    t_jgraphics *g) {
...
bool presetter_in_confirm_cancel_button_bounds(t_presetter *p, t_rect *rect,
                                               t_pt *pt, t_jgraphics *g) {
...
void presetter_mousedown(t_presetter *p, t_object *patcherview, t_pt pt,
  ...
  if (presetter_in_grid_bounds(p, &rect, &pt)) {
    ...
    if (modifiers & eLeftButton && (modifiers & eShiftKey) &&
      ...
      snprintf_zero(p->j_confirm_status_text, sizeof(p->j_confirm_status_text),
                    "Delete Preset %d?", cell_idx);
      jbox_redraw((t_jbox *)p);
      return;
    }
    ...
    if (modifiers & eLeftButton && modifiers & eShiftKey &&
      ...
      p->j_status_override = PRESETTER_CONFIRM_STATUS;
      snprintf_zero(p->j_confirm_status_text, sizeof(p->j_confirm_status_text),
                    "Store Preset %d?", cell_idx);
      jbox_redraw((t_jbox *)p);
      return;
    }
  }

  if ((p->j_confirm_store || p->j_confirm_delete) &&
      p->j_confirm_status_text[0] != '\0' && p->j_confirm_cell != -1) {
...
void presetter_mousemove(t_presetter *p, t_object *patcherview, t_pt pt,
  ...
  if (presetter_in_grid_bounds(p, &rect, &pt)) {
    ...
    if (name && name != gensym("<(unnamed)>")) {
      snprintf_zero(p->j_hover_status_text, sizeof(p->j_hover_status_text),
    ...
    if (p->j_status == PRESETTER_HOVER_STATUS ||
        p->j_hovered_cell != p->j_last_hovered_cell) {
...
long presetter_key(t_presetter *p, t_object *patcherview, long keycode,
  ...
  if (p->j_editing_name) {
    ...
    if (textcharacter >= 32 && textcharacter < 127 &&
        len < sizeof(p->j_preset_name) - 1) {
...
void presetter_draw_preset_name(t_presetter *p, t_jgraphics *g, t_rect *rect) {
  ...
  if (p->j_editing_name) {
    ...
    jcolor_getcolor(PRESET_NAME_EDITING_BG_COLOR_SYM, &bg_color,
                    &_bg_color_off);
  ...
  if (p->j_preset_name[0] != '\0') {
    if (p->j_editing_name) {
      jcolor_getcolor(PRESET_NAME_TEXT_EDITING_COLOR_SYM, &name_color,
                      &_name_color_off);
    } else {
      jcolor_getcolor(PRESET_NAME_TEXT_DEFAULT_COLOR_SYM, &name_color,
                      &_name_color_off);
    }
  } else {
    presetter_hex_to_rgba(&name_color, PRESET_NAME_TEXT_UNSELECTED_COLOR_HEX,
                          1);
  }
...
void presetter_draw_write_button(t_presetter *p, t_jgraphics *g, t_rect *rect) {
  ...
  if (p->j_write_button_down) {
    ...
    jcolor_getcolor(WRITE_BUTTON_ON_TEXT_COLOR_SYM, &text_color,
                    &text_color_off);

  } else if (p->j_editing_name) {
    jcolor_getcolor(WRITE_BUTTON_UP_BG_COLOR_SYM, &bg_color, &bg_color_off);
    jcolor_getcolor(WRITE_BUTTON_UP_TEXT_COLOR_SYM, &text_color,
                    &text_color_off);
  ...
  double text_y =
      bounds.y + (bounds.height + extents.ascent - extents.descent) / 2;
...
void presetter_draw_row(t_presetter *p, t_jgraphics *g, t_grid_dim *dim,
                        int row_idx, double y) {
...
const char *presetter_get_status_text(t_presetter *p) {
  t_status effective = (p->j_status_override != PRESETTER_NO_STATUS)
                           ? p->j_status_override
                           : p->j_status;
  ...
  if (effective == PRESETTER_CONFIRM_STATUS &&
      p->j_confirm_status_text[0] != '\0')
...
void presetter_draw_confirm_ok_button(t_presetter *p, t_jgraphics *g,
                                      t_rect *rect) {
  if (p->j_confirm_status_text[0] == '\0' ||
      !(p->j_confirm_store || p->j_confirm_delete))
    return;
  ...
  if (p->j_confirm_ok_button_down) {
    ...
    jcolor_getcolor(CONFIRM_BUTTON_ON_TEXT_COLOR_SYM, &text_color,
                    &text_color_off);
  } else {
...
void presetter_draw_confirm_cancel_button(t_presetter *p, t_jgraphics *g,
                                          t_rect *rect) {
  if (p->j_confirm_status_text[0] == '\0' ||
      !(p->j_confirm_store || p->j_confirm_delete))
    return;
